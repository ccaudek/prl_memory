...
reward_probabilities = {
    ...
    "white_rewarded_first": [(0.1, 0.9), (0.9, 0.1)]   # (epoch1: (orange, white), epoch2: (orange, white))
}
    ...
    while key_pressed is None:
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN and event.key in response_keys:
                ...
                time.sleep(0.5)
                return orange_pos, key_pressed, reaction_time, is_correct, last_video_played



# Function to save trial results to CSV, modified to include video file name
def save_trial_results(trial_results, filename='experiment_results.csv', mode='a'):
...
# Initially save with a header
save_trial_results([], mode='w')  # This creates the file with the header only
...
for epoch in range(n_epochs):
    for trial in range(trials_per_epoch):
        ...
        orange_pos, key_pressed, reaction_time, is_correct, video_played = display_images_and_get_response(orange_img, white_img, epoch, last_video_played)

        trial_results.append([epoch, trial, orange_pos, key_pressed, reaction_time, is_correct, video_played])
        
        # Save every 5 trials, plus ensure the last set of trials is saved
        if (trial + 1) % 5 == 0 or trial == trials_per_epoch - 1:
            save_trial_results(trial_results)
            trial_results = []  # Reset trial results after saving
...
with open('experiment_results.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Epoch', 'Trial', 'Orange Position', 'Key Pressed', 'Reaction Time', 'Is Correct', 'Last Video Played'])
    writer.writerows(trial_results)
