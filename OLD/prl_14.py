...
def display_images_and_get_response(orange_img, white_img, epoch, orange_file, white_file):
    ...
    image_left, image_right = (orange_file, white_file) if positions[0] == 'Left' else (white_file, orange_file)

    orange_pos = positions[0]
    white_pos = positions[1]

    orange_image_index = orange_images.index(orange_img)
    white_image_index = white_images.index(white_img)
    ...
    chosen_image = orange_image_files[orange_image_index] if key_pressed == 'Left' else white_image_files[white_image_index]
    return orange_pos, key_pressed, reaction_time, chosen_image, image_left, image_right
...
for epoch in range(n_epochs):
    ...
    for trial in range(trials_per_epoch):
        ...
        is_correct = determine_reward(epoch, 'Orange' if key_pressed == 'Left' else 'White')
        ...
        trial_data = {
            ...
            "Most Rewarded Stimulus in Epoch": 'Orange' if epoch % 2 == 0 else 'White',
