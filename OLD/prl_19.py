...
def display_mood_slider():
    ...
    slider_changed = [False]  # Use list for mutability in closure

    mood_text = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect((50, 50), (900, 50)),
        text="Come ti senti in questo momento?",
        manager=manager,
    )

    mood_slider = pygame_gui.elements.UIHorizontalSlider(
        relative_rect=pygame.Rect((50, 150), (900, 50)),
    ...
    continue_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((screen_width / 2 - 50, 300), (100, 50)),
        text="Continue",
        manager=manager,
        visible=False,  # Initially hidden
    )

    def check_slider_changed():
        if mood_slider.get_current_value() != initial_value:
            slider_changed[0] = True
    ...
    # Capture the slider's value before exiting the function
    mood_value = mood_slider.get_current_value()
    return mood_value  # Return the value for saving
