...
# Define parameters
condition = "surprise"  # Choose between "surprise" or "nosurprise"
n_epochs = 2
trials_per_epoch = 20  # Adjusted to accommodate 41+ trials
...
videos_to_play = select_videos() if condition == 'surprise' else []
...
def display_images_and_get_response(orange_img, white_img):
    screen.fill(WHITE)
    ...
    while key_pressed is None:
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN and event.key in response_keys:
                ...
                # Display feedback
                feedback_color = GREEN if key_pressed == orange_pos else RED
                feedback_text = 'Correct!' if key_pressed == orange_pos else 'Wrong!'
                ...
                time.sleep(0.5)  # Show feedback for 2 seconds
                return orange_pos, key_pressed, reaction_time
...
for epoch in range(n_epochs):
    for trial in range(trials_per_epoch):
        ...
        orange_pos, key_pressed, reaction_time = display_images_and_get_response(orange_img, white_img)

        trial_results.append([epoch, trial, orange_pos, key_pressed, reaction_time])
...
with open('experiment_results.csv', 'w', newline='') as csvfile:
    ...
    writer.writerow(['Epoch', 'Trial', 'Orange Position', 'Key Pressed', 'Reaction Time'])
