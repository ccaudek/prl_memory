...
def save_data(trial_data, filename='experiment_data.csv', mode='a'):
    fieldnames = ["Trial Number", "Epoch", "Stimulus Position", "Key Pressed", "Reaction Time", "Feedback Received", "Chosen Image", "Most Rewarded Stimulus in Epoch", "Image Left", "Image Right"]
...
save_data([], mode='w')  # Create the file with the header
...
def display_images_and_get_response(orange_img, white_img, epoch, orange_file, white_file):
    screen.fill(WHITE)
    ...
    chosen_image_file = image_left_file if key_pressed == response_keys[pygame.K_f] else image_right_file

    return positions[0], key_pressed, reaction_time, chosen_image_file, image_left_file, image_right_file
...
for epoch in range(n_epochs):
    ...
    for trial in range(trials_per_epoch):
        ...
        orange_pos, key_pressed, reaction_time, chosen_image, image_left, image_right = display_images_and_get_response(orange_img, white_img, epoch, orange_file, white_file)
        
        # Determine the chosen color for feedback
        chosen_color = 'Orange' if key_pressed == 'Left' and orange_pos == 'Left' or key_pressed == 'Right' and orange_pos == 'Right' else 'White'
        ...
        if is_correct:
            screen.blit(happy_img, (screen_width / 2 - happy_img.get_width() / 2, screen_height / 2 - happy_img.get_height() / 2))
            play_sound('beeps/pleasant.wav')
        else:
            screen.blit(sad_img, (screen_width / 2 - sad_img.get_width() / 2, screen_height / 2 - sad_img.get_height() / 2))
        ...
        trial_data = {
            ...
            "Most Rewarded Stimulus in Epoch": 'Orange' if (condition == "orange_rewarded_first" and epoch == 0) or (condition == "white_rewarded_first" and epoch == 1) else 'White',
